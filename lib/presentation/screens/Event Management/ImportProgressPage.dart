...
class ImportEventProgressPage extends StatefulWidget {
  final String filePath;

  const ImportEventProgressPage({super.key, required this.filePath});

  @override
  State<ImportEventProgressPage> createState() =>
      _ImportEventProgressPageState();
}

class _ImportEventProgressPageState extends State<ImportEventProgressPage> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<EventCubit>().importFromExcel(widget.filePath);
    });
  }
    ...
    return Scaffold(
      ...
      body: BlocConsumer<EventCubit, EventState>(
        listenWhen: (previous, current) =>
            current is EventImportedReport || current is EventError,
        ...
        buildWhen: (previous, current) {
          return current is ImportProgress ||
              current is EventImportedReport ||
              current is EventError;
        },
        builder: (context, state) {
          if (state is ImportPreparing) {
            return _buildPreparing(); // شاشة التحضير
          } else if (state is ImportProgress) {
            return _buildProgress(state);
          } else if (state is EventImportedReport) {
  ...
  Widget _buildPreparing() {
    return Center(
      child: Column(
        ...
        children: [
          Image.asset("assets/img/loading.png", height: 200),
  ...
  Widget _buildReport(EventImportedReport state) {
    return Center(
